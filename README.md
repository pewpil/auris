...
- Project schedule (Gantt, 2026-09-06 → 2027-04): [`docs/gantt.md`](docs/gantt.md)
- Concept: [`.opencode/concept.md`](.opencode/concept.md) · plan summary for AI sessions: [`.opencode/plan.md`](.opencode/plan.md)
- Thesis: writing plan & status tracker [`docs/auris-thesis/README.md`](docs/auris-thesis/README.md) · draft [`docs/auris-thesis/paper.md`](docs/auris-thesis/paper.md) · IEEE mirror (scaffolded) [`docs/auris-thesis/ieee.md`](docs/auris-thesis/ieee.md)
...
|---|---|---|---|---|
| Intel RealSense **D435i** (D9) | RGB-D + IMU sensor | RealSense store (US$334, ships to PH); RX Electronics PH (US$343.75–355, HK-shipped) | ≈ ₱21,000–22,200 + freight/clearance → **≈ ₱24,000–27,000 landed** | native USB, 90 FPS depth, head-mount friendly; **built-in IMU — buy the "i" variant specifically** (pose estimation requires it); precedented by Fei et al. 2024 and Lee & Medioni 2016. **Over OAK-D Lite** (~US$149, verified vs Luxonis docs 2026-09-04): the Lite is passive-stereo only (no IR dot projector → invalid depth on the textureless walls a room map depends on), its onboard Myriad-X AI is unused by this architecture (detection runs on the desktop, D2/D8), and `realsense2_camera` + `rtabmap_ros` is the canonical pairing for the Phase 7b port; the Lite's advantages — price, 13 MP autofocus RGB, current-batch BMI270 IMU (Kickstarter units shipped IMU-less) — do not outweigh those points here |
...
| Compressed Semester 1 (pre-oral before 2026-12-14)                                                                     | Phases 0–5 + the evaluation + Chapters 1–5 must all land in one semester; a slip cascades into the pre-oral | 6a (protocol/ethics/recruitment) runs in parallel from late September; overlaps (2/3/4/5, 6b/6c, Ch4/Ch5) absorb slip; the recorder (D8) allows offline analysis catch-up; protect the pre-oral before any Phase 7 work ([`docs/gantt.md`](docs/gantt.md))                                                     |
...
- **2026-09-07 (rev. 9)** — **Schedule re-planned around the real defense calendar** ([`docs/gantt.md`](docs/gantt.md)): the **pre-oral defense moves before Semester 1 ends (≈ 2026-12-11, tentative)** on a full Chapters 1–5 draft whose **Chapters 4–5 draw on the prototype evaluation — Phase 6 (evaluation study) now runs in Semester 1**, immediately after the Phase 5 gate (Phases 0–5 compressed with overlap; 6a protocol/ethics/recruitment in parallel from late September). The **Production iteration (Phase 7) runs entirely in Semester 2** (procurement → 7a → 7b → 7c, Jan–Mar 2027), feeding a Chapter 4 production-revalidation update before the **final defense 2027-04-15** (before Semester 2 ends); revisions to 2027-04-30. Thesis writing-plan dates re-mapped ([`docs/auris-thesis/README.md`](docs/auris-thesis/README.md)); compressed-semester risk row added ([§10](#10-risks-and-mitigations)).

- **2026-09-07 (rev. 8)** — **Thesis-writing conventions restructured** ([`.opencode/man.md`](.opencode/man.md)): man.md is now a symlink to the canonical [`docs/auris-thesis/manuscript.md`](docs/auris-thesis/manuscript.md), which merges the current conventions (draft = `paper.md`, IEEE twin duty, numbering rules) and drops the stale pre-consolidation drafting-schedule table (scheduling lives in [`docs/auris-thesis/README.md`](docs/auris-thesis/README.md)); [`docs/auris-thesis/ieee.md`](docs/auris-thesis/ieee.md) rebuilt as a heading-exact mirror of `paper.md` (IEEE standards govern the writing itself) after the original scaffold was lost to a `git reset --hard` in the nested `pewpil/auris-thesis` repo; old single-file draft `thesis.md` retired (recoverable from that repo's history).

- **2026-09-07 (rev. 7)** — **Academic calendar corrected** ([`docs/gantt.md`](docs/gantt.md)): Semester 1 ends **2026-12-14** per the school calendar (was the assumed 2026-12-18 one-week-before-Christmas date); semester break runs 2026-12-15 → 2027-01-03; the Phase 5 close and Chapter 3 draft tail trim to the new boundary. Defense schedule unchanged — **pre-oral ≈ 2027-03-22 (full Chapters 1–5 draft) falls inside Semester 2, before that semester ends** (2027-04-15); final defense on 2027-04-15.

- **2026-09-04 (rev. 6)** — **Thesis writing plan finalized** ([`docs/auris-thesis/README.md`](docs/auris-thesis/README.md)): section-by-section source map, RQ set (RQ1–RQ4, mirrored by Objectives), Chapter 2 theme organization + chase list, full Chapter 3 Mermaid figure set, canonical glossary, per-chapter quality gates, and the writing schedule — Chapter 2 drafting opens 2026-09-08; **pre-oral defense ≈ 2027-03-22 (tentative) on a full Chapters 1–5 draft with Chapter 4 from the prototype evaluations** (no proposal defense); final defense 2027-04-15. [`docs/gantt.md`](docs/gantt.md) redrawn accordingly; [`docs/auris-thesis/ieee.md`](docs/auris-thesis/ieee.md) scaffolded (per-chapter IEEE mirror per man.md).

- **2026-09-04 (rev. 5)** — **Project Gantt added** ([`docs/gantt.md`](docs/gantt.md)), as **one chart per semester with phases as the section axis** (plus a completed old-concept chart): new-concept development starts 2026-09-06; Semester 1 (dev Phases 0–5 + thesis Ch. 1–3 draft + proposal defense) ends 2026-12-18; break to 2027-01-03; Semester 2 (evaluation study, Production 7a–7c, thesis Ch. 3–5 + ieee.md) ends 2027-04-15 with the final defense; revisions complete April 2027.
