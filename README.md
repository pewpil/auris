...
Paradigm justified by the consolidated literature ([`docs/auris-thesis/literature/07-evaluation-methodology.md`](docs/auris-thesis/literature/07-evaluation-methodology.md)). The participant model changed 2026-09-16 and is **gated** (thesis plan decision 2):

- **Primary design**: a **longitudinal single-case study with VI participant(s)** (participant count under discussion) — stable aid-off baseline (~4 sessions) then 4–6 weeks aided (~8–12 sessions, ~2/week) over 3–4 layouts randomized per session; single-case analysis — visual phase comparison + non-overlap indices (Kratochwill et al. 2010; Horner et al. 2005; Parker et al. 2011). Concept-faithful target user.
- **Fallback**: blindfolded-sighted N = 10–15 within-subject aid-off vs aid-on, counterbalanced condition × layout order; validity support: dos Santos et al. 2021 (blindfolded users are slower → conservative results), Giudice et al. 2020 (sighted controls as best-case bound).
- **Gates**: adviser/panel approval of the single-participant design, and a confirmed eligible participant by Dec 15, 2026; either gate failing by early January executes the fallback unchanged.
- **Course**: obstacle room course in the style of Roentgen et al. 2012b (standardized layouts, randomized per session).
- **Conditions**: aid-off baseline vs. aid-on — phase-wise (baseline → aided) under the primary design; counterbalanced under the fallback; multiple layouts.
- **Metrics**:
...
| **P5 — Pilot evaluation** | human study on the room course — primary: longitudinal single-case with VI participant(s), gated (adviser approval + participant confirmed by Dec 15, 2026; baseline → 4–6 weeks aided); fallback: blindfolded-sighted N = 10–15 within-subject | metrics + questionnaires collected; single-case phase analysis under the primary design, group comparison under the fallback | §4 (results and discussions) |
...
- **Purchase list** — [`docs/purchase-list.md`](docs/purchase-list.md): the approved-now P2 bench order (everything T0–T8 consume), the pre-soldered ordering rule, the totals, and the checkout checklist.
...
- **Literature consolidation** — [`docs/auris-thesis/literature/README.md`](docs/auris-thesis/literature/README.md): 50 verified annotated entries across 7 themes; feeds thesis §2.
...
- **Budget & purchase approval** — full hardware requirements with Philippine-market pricing are complete in [§3](#3-hardware) (researched 2026-09-08); the remaining open items are the own-printer vs. print-service decision and the purchase-approval sign-off itself; the pointer-tracking hardware (✚ rows in [§3.1](#31-pointer--electronics)/[§3.2](#32-wearable--electronics), ≈ ₱3,400–6,600 *(est)*, contingencies in [§3.8](#38-pointer-tracking-hardware--notes--contingencies)) is inside the [§3.6](#36-totals) totals. Timing decided 2026-09-16: sign-off lands within days of that date with ordering by Sep 20, 2026, so the breadboard-bench phase starts on the [combined schedule](docs/schedule.md); the sign-off itself remains the open action. The approved-now order is the P2 bench purchase list ([`docs/purchase-list.md`](docs/purchase-list.md), built 2026-09-17): everything T0–T8 of [`docs/bench-tests.md`](docs/bench-tests.md) consume — frozen-core electronics, the ✚ tracking hardware, and the T7/T8 hedge boards — with P3-only blocks deferred (§13 there).
- **Thesis writing schedule** — resolved 2026-09-11: the section→phase drafting plan is in [`docs/auris-thesis/README.md`](docs/auris-thesis/README.md), integrated with the phase exits above; re-based 2026-09-16 onto the two defense milestones (pre-oral Fri Dec 11, 2026; final Tue Apr 20, 2027) in [`docs/schedule.md`](docs/schedule.md).
- **Participant-model change (2026-09-16)** — the thesis study's primary design is a longitudinal single-case study with VI participant(s) (participant count under discussion; thesis plan decision 2); two gates: adviser/panel approval of the design (immediate action) and a confirmed eligible participant by Dec 15, 2026 (outreach via VI organizations/schools — Resources for the Blind Inc., ATRIEV, NCDA network; ethics/consent in accessible formats); either gate failing by early January 2027 executes the documented blindfolded-sighted fallback ([§5](#5-evaluation-plan), [`docs/schedule.md`](docs/schedule.md)).
